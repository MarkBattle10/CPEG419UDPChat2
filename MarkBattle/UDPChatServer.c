...
int main(int argc, char *argv[])
{
	...
	char sendToUser[MSGSIZE]; /* holds the name of the user that the other user wants to send a message privately to */
	char sendToUser2[MSGSIZE];
	...
	for (;;) /* Run forever */
	{
		 ...
		 */
			...
			bzero(sendToUser2, MSGSIZE);
			strcpy(sendToUser2, sendToUser);
			bzero(requestMsgBuf, MSGSIZE);
			//printf("sendToUser2: %s\n", sendToUser);
			if((recvMsgSize = recvfrom(sock, requestMsgBuf, MSGSIZE, 0, (struct sockaddr *) &chatClntAddr, &cliAddrLen))<0){
				DieWithError("recvfrom() failed");
			}
			//printf("sendToUSer2: %s\n", sendToUser);
			requestMsgBuf[recvMsgSize] = '\0';
			//printf("sendToUser3: %s\n", sendToUser);
			strcat(sendMsgBuf, requestMsgBuf);
